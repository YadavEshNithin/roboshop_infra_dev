...
resource "aws_lb_listener" "frontend_alb_fr" {
  load_balancer_arn = module.alb.arn
  port              = "443"
  protocol          = "HTTPS"
  ssl_policy        = "ELBSecurityPolicy-2016-08"
  certificate_arn   = local.acm_listener_arn
  ...
  default_action {
    ...
    fixed_response {
      ...
      message_body = "<h1> hello from iam from the frontend alb og's sir's student </h1>"
      status_code  = "200"
    }
  }
}
